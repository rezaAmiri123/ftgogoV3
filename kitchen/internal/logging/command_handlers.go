...
func LogCommandHandlersAccess[T ddd.Command](handlers ddd.CommandHandler[T], label string, logger zerolog.Logger) ddd.CommandHandler[T] {
...
func (h CommandHandlers[T]) HandleCommand(ctx context.Context, cmd T) (ddd.Reply, error) {
	h.logger.Info().Msgf("--> Kitchen.%s.On(%s)", h.label, cmd.CommandName())
	defer func() { h.logger.Info().Msgf("<-- Kitchen.%s.On(%s)", h.label, cmd.CommandName()) }()
